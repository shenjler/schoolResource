...
<%
	int collegeLiId=Integer.parseInt(request.getParameter("collegeLiId"));
	int courseId2=Integer.parseInt(request.getParameter("courseId")); 
	String collegeLiName=null;
	String courseLiName=null;
	if (collegeLiId != 0) {
		CcgcDAO cd = new CcgcDAOImpl();
		collegeLiName=cd.queryCollegeName(collegeLiId);
		courseLiName=cd.queryCourseName(courseId2);
	}
%>
<!--导航-->
            <div class="avr">
...
<div class="xg_ban">
  ...
  <div class="middle">
   ...
   <%			
				...
				//统计总记录数
				int allcount = 0;//总记录数
				allcount = fsd.findCourseFileCount(collegeLiId,courseId2);
				//设置每一页要显示多少条记录
				...
				List<FileSearch> all = fsd.queryByCourse(currpage,everpage,collegeLiId,courseId2);
    ...
    <!--中间右边-->
    	<div class="right">
             ...
             <div class="table"> 
             		...
             		if(allpages>1){
             		%>
                      <a href="college2.jsp?pages=<%=currpage-1%>&courseId=<%=courseId2%>&collegeLiId=<%=collegeLiId %>" class="gray">&lt;</a>
               	   <!-- <span id="a1 " class="gray">&lt;</span> -->
               	 <% 
           				for(int i=1;i<=allpages;i++)
             			{
            	 %>
            	 <a href="college2.jsp?pages=<%=i%>&courseId=<%=courseId2%>&collegeLiId=<%=collegeLiId %>" 
            	<%if(currpage==i){%>
            	 class="yellow"
            	 <% }else{ %> 
            	 class="gray"
